...
class DetThread(QThread):
    ...
    def __init__(self):
        ...
        self.percent_length = 1  # progress bar
        ...
        try:
            ...
            while True:
                if self.jump_out:
                    ...
                    break
                # change model
                if os.path.basename(self.current_weight) != os.path.basename(self.weights):
                    # Load model
                    model = attempt_load(self.weights, map_location=device)  # load FP32 model
                    num_params = 0
                    for param in model.parameters():
...
class MainWindow(QMainWindow, Ui_mainWindow):
    def __init__(self, parent=None):
        ...
        self.comboBox.clear()
        pos1_pt = self.det_thread.weights
        self.pt_list = os.listdir('./pt')
        self.pt_list = [file for file in self.pt_list if file.endswith('.pt')]
        self.pt_list.sort(key=lambda x: os.path.getsize('./pt/' + x))
        self.comboBox.clear()
        self.comboBox.addItem(pos1_pt)
        print(self.pt_list)
        self.comboBox.addItems(self.pt_list)
        self.qtimer_search = QTimer(self)
        self.qtimer_search.timeout.connect(lambda: self.search_pt())
        self.qtimer_search.start(200)
    ...
    def chose_cam(self):
        try:
            self.stop()
            MessageBox(
                self.closeButton, title='Tips', text='Loading camera', time=1000, auto=True).exec_()
            ...
            self.det_thread.source = str(cams[0])
            self.statistic_msg('Loading camera：{}'.format(str(cams[0])))
            if action:
